...
This project was to build a desktop app that can "harvest" from the CRKN websites the spreadsheets with that data, and also ingest locally maintained spreadsheets following the same structure. Then it would provide an easy to use GUI to search that data and keep track of the PAR licenses in case the library needs to dispute their rights with any provider/publisher.
